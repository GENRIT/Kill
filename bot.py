...
async def start_mailing(message, group_id):
    ...
    while True:
        try:
            if sent_message:
                await bot.delete_message(group_id, sent_message.message_id)

            mention_text = create_mention_text(text, recipients)
            sent_message = await bot.send_message(group_id, mention_text, parse_mode='HTML')
            
            # Ждем 0.1 секунды перед следующим обновлением
    ...
    # Отправляем финальное сообщение без упоминаний
    try:
...
def create_mention_text(text, recipients):
    mention_text = ""
    for i, char in enumerate(text):
        if i < len(recipients):
            mention_text += f'<a href="tg://user?id={recipients[i]}">{char}</a>'
        else:
            mention_text += char
    return mention_text
